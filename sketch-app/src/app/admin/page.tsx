...
// CELEB SKETCH 관리 — 내부 운영 도구(한국어 전용). 전용 ADMIN_KEY 로그인 (match 콘솔과 크레덴셜 분리).
import { useEffect, useState } from "react";
import { ShieldCheck, Palette, ScrollText, ExternalLink } from "lucide-react";
...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<TabKey>("sketch");

  // 세션 유효성 확인 (logs 401 여부)
  useEffect(() => {
    aget("/api/admin/logs?limit=1")
      .then(() => setAuthed("yes"))
      .catch(() => setAuthed("no"));
  }, []);
      ...
      <main className="admin-shell flex min-h-[100dvh] items-center justify-center px-6">
        <div className="w-full max-w-sm rounded-[18px] bg-surface-1 p-8 ring-1 ring-hairline">
          <div className="flex flex-col items-center">
            ...
            <h1 className="mt-3 text-[18px] font-black">CELEB SKETCH 관리</h1>
    ...
    <div className="admin-shell flex min-h-[100dvh]">
      ...
      <aside className="sticky top-0 hidden h-[100dvh] w-[220px] shrink-0 flex-col border-r border-hairline bg-surface-1/60 px-3 py-5 lg:flex">
        <div className="flex items-center gap-2 px-2">
          ...
          <div>
            <div className="text-[14px] font-black leading-tight">CELEB SKETCH</div>
        ...
        <div className="mt-auto flex flex-col gap-2 px-2">
          ...
          >
            <ExternalLink className="h-3.5 w-3.5" /> 앱 열기
          </a>
      ...
      <main className="min-w-0 flex-1">
        ...
        <div className="border-b border-hairline px-4 pb-3 pt-4 lg:hidden">
          <div className="flex items-center gap-2">
            <ShieldCheck className="h-5 w-5 text-primary-400" />
            <span className="text-[15px] font-black">CELEB SKETCH 관리</span>
          </div>
