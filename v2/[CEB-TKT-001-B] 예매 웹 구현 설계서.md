...
예매 웹 (Next.js, Vercel)
   ...
   ├─ /orders      주문 내역 (A5)
   │               (내 티켓 메뉴 없음 — 지급 확인은 예매내역 4단계 타임라인, 무상 지급분은 예매내역 하단 섹션. §5 역할 분담)
   ├─ /report      암표 신고 (A7)
...
- 예매 웹: 회원 탭은 홈·예매내역·신고 3개 — **"내 티켓" 메뉴 없음** (2026.09.01 개편). 지급 확인은 예매내역의 **4단계 진행 상태(예매 접수 → 입금 대기 → 입금 확인 → 티켓 지급)**로 제공하고, 무상 지급 티켓(래플 당첨·초대)은 예매내역 하단 "지급받은 티켓" 섹션에서 확인. 관리자 체크인 화면 없음
...
| (내 티켓 화면 없음) | — | A6 폐지 (2026.09.01) — 지급 확인은 `/orders` 타임라인·"지급받은 티켓" 섹션으로 통합. `/app/tickets` 접근은 예매내역으로 이동. 티켓 조회 API는 본 앱 연동·섹션 데이터용으로 유지 |
