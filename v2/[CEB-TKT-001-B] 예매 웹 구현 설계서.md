...
예매 웹 (Next.js, Vercel)
   ...
   ├─ /orders      주문 내역 (A5)
   ├─ /tickets     내 티켓 — 지급 상태 확인 (A6. 발권 QR·체크인은 CELEBUS 본 앱 담당 — §5 역할 분담)
   ├─ /report      암표 신고 (A7)
...
- 예매 웹: 회원 "내 티켓"은 지급 상태 확인용(QR·코드 비노출, "입장 QR은 CELEBUS 앱에서" 안내). 관리자 체크인 화면 없음
...
| `/tickets` `/tickets/[id]` | A6 | 지급 상태 확인 전용 — QR·코드 비노출, 입장은 CELEBUS 앱 안내 (§5 역할 분담) |
