...
Supabase (단일 프로젝트, dev/prod 분리)
   ...
   ├─ Auth: 회원·관리자 세션
   └─ 스토리지: 신고 증빙 이미지(비공개 버킷, 서버 경유 업로드·서명 열람) + 공연 포스터(공개)
   │
   ▼ 외부
...
| v1.8 | 2026.09.02 | @Nill Yoo | 신고 증빙 이미지 업로드 신설 — 회원이 신고 접수 시 이미지 최대 3장(장당 5MB, jpeg/png/webp) 첨부 가능. 증빙은 민감 자료로 비공개 저장소에 서버 경유 업로드(회원 직접 쓰기 금지), 운영자는 신고 처리 화면에서 1시간 서명 주소로만 열람. 기존 "증빙 이미지 2차" 보류를 구현으로 전환 |
