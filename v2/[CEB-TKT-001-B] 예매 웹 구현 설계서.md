...
예매 웹 (Next.js, Vercel)
   ├─ /            공연 목록·상세 (A1·A2)
   ├─ /verify      간편인증 (A3) ──────────► 간편인증 대행사 (PASS·카카오·토스·네이버)
...
- 연동: 간편인증 대행사(예: 포트원 통합인증) 경유로 PASS·카카오·토스·네이버 일괄 제공 — **DI 반환 조건 필수 확인** (계약 트랙, [CEB-TKT-001-A] §8.3).
...
**QR·체크인**: 티켓 `code`는 서버 생성 난수(8자+). 티켓 화면은 열 때마다 서버에서 **서명된 단기 토큰**(코드+만료 포함, 수십 초 유효)을 받아 QR 렌더 — 캡처 공유 무력화 경량안([CEB-TKT-001-A] §4.7). 스캐너(관리자 `/admin/checkin`)는 토큰 검증 → `check_in_ticket` 호출. 오프라인 폴백: 코드 직접 입력.
...
| `/tickets` `/tickets/[id]` | A6 | QR = 서명 토큰 방식(§5). 시간 게이팅은 서버 시각 |
...
| `/admin/refunds` `/admin/checkin` `/admin/reports` `/admin/logs` | B3·B4·B5·로그 | SLA 타이머·중복 스캔 경고 동일 |
...
### 8.1 간편인증 (오픈 전 필수)
대행사 통합인증(포트원 등) 우선 견적 — 4개 수단 일괄 + DI 반환 + 심사 리드타임 확인. 계약 지연 시 폴백: 본인확인기관 1개사(휴대폰 본인확인) 선오픈 후 수단 확대.
