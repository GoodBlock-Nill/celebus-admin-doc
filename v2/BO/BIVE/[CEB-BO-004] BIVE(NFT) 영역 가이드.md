...
| **BIVE 미디어** (v1.10) | **앞면(필수) + 뒷면(선택) 2장 분리**. 각 PNG·JPG·GIF·WebP·MP4 / 최대 20MB. NFT 카드 양면 표준 패턴 (앞면 일러스트 + 뒷면 패턴/스토리). [202-CREATE] §2-2 우측 패널 grid-cols-2 (좌우 박스 2개, 입력 효율). [202] InfoTab 미디어 카드 세로 스택 (1열 1장씩, 미리보기 가시성 우선) |
...
| **민팅 이벤트 연결 기능** | 캠페인 ↔ 연결 기능은 **1:1**. 개발 프리셋에서 내려주는 목록만 사용. 운영 BO 실제 Event 탭 4종 — **회원가입 보상 / 출석체크 보상 / 래플 보상 / 팬퀘스트 보상**. Ticket·Mix·Pick 탭은 별도 프리셋 |
