...
| **보상 방식 배지 노출 표준** (v1.8) | 가중치 보상 = `bg-indigo-100 text-indigo-700` "가중치" / 지정 보상 = `bg-emerald-100 text-emerald-700` "지정". 노출 위치 — [103] 민팅 이벤트 리스트 테이블 + [202] BIVE 상세 §민팅관리 탭 테이블 (캠페인명 옆). 운영자가 한눈에 구별 가능 |
