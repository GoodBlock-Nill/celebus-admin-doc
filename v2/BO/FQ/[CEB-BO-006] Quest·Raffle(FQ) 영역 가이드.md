# [CEB-BO-006] Quest(FQ) 영역 가이드

> ⚠️ **영역 분리 (v1.2, 2026-05-13)** — 본 문서는 v1.1까지 Quest와 Raffle을 함께 다뤘으나, 사이드바 메뉴 재구성으로 **Raffle은 [CEB-BO-012] RFL 독립 영역**(`v2/BO/RFL/`)으로 이전되었습니다. 본 문서는 이제 **Quest 영역 전용**입니다. Raffle 명세는 [CEB-BO-012] 및 `[CEB-BO-RFL-101 ~ 203-MD-NOTE]` 참조.
...
| 버전 | v1.2 |
| 최근 업데이트 | 2026.05.13 |
| 비고 | v1.1 — 사이드바 메뉴 재구성: Quest는 "에피소드" 그룹 하위 페이지(`/sq/quests`), Raffle은 "래플" 그룹(`/raffle`)으로 분리. 응모권 그룹 폐기 → "래플" 그룹으로 흡수 |
...
| --- | --- | --- | --- | --- | --- |
| CEB-BO-FQ-101 | 팬퀘스트 (Quest 리스트) | `/sq/quests` | Page | v1 흡수 + v2 정책 호환 | ✅ v1.1 |
| CEB-BO-FQ-102 | 래플 (Raffle 리스트) | `/raffle` | Page | v1 흡수 + v2 정책 호환 | ✅ v1.1 (스텁) |

### 2.2 Quest 생성·생성 모달 (5)

| 화면ID | 화면명 | 유형 | 마이그레이션 상태 |
| --- | --- | --- | --- |
| CEB-BO-FQ-101-CREATE | Quest 생성 | Page | 후속 |
| CEB-BO-FQ-101-MD-CANCEL | Quest 생성 취소 | Modal | 후속 |
| CEB-BO-FQ-101-MD-COMPLE | Quest 생성 완료 | Modal | 후속 |
| CEB-BO-FQ-101-MD-ADD | 반려사유 추가 | Modal | 후속 |
| CEB-BO-FQ-101-MD-EDIT | 반려사유 수정 | Modal | 후속 |

### 2.3 Quest 상세·수정·검수 (8)

| 화면ID | 화면명 | 유형 | 마이그레이션 상태 |
| --- | --- | --- | --- |
| CEB-BO-FQ-201 | Quest 상세 (기본정보 탭) | Page | 후속 |
| CEB-BO-FQ-202 | Quest 상세 (대기내역 탭) | Page | 후속 |
| CEB-BO-FQ-203 | Quest 상세 (처리내역 탭) | Page | 후속 |
| CEB-BO-FQ-201-EDIT | Quest 수정 | Page | 후속 |
| CEB-BO-FQ-201-MD-OPEN | Quest 게시 | Modal | 후속 |
| CEB-BO-FQ-201-MD-CLOSE | Quest 종료 | Modal | 후속 |
| CEB-BO-FQ-201-MD-DELETE | Quest 삭제 | Modal | 후속 |
| CEB-BO-FQ-202-MD-REJECT | Quest 반려사유 | Modal | 후속 |

### 2.4 Quest 부속 모달 (3)

| 화면ID | 화면명 | 유형 |
| --- | --- | --- |
| CEB-BO-FQ-201-MD-CANCEL | Quest 수정 취소 | Modal |
| CEB-BO-FQ-201-MD-COMPLE | Quest 수정 완료 | Modal |
| CEB-BO-FQ-101-REJETSET | 반려사유 설정 | Page |
