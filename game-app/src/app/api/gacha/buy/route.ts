...
const buySchema = z.object({ qty: z.number().int().min(1).max(99).default(1) });

// 유상 가챠 이용권 구매 (CP 차감) — 가격·잔액은 서버 RPC가 원자적으로 검증.
// 유상 이용권은 재화 가챠 전용(사행성 분리) — 실물 가챠에는 사용 불가.
export async function POST(req: Request) {
  ...
  const { data: result, error } = await admin().rpc("game_gacha_buy_ticket", {
