...
### 3-1-1. 운영 확률 설계 가이드 (2026-08-12 확정, 2026-08-13 확률 상향 — 콘서트 모바일 티켓 이벤트)

**확정 사항**: 모바일 티켓 총 40장 = **최대 지급량(예산 상한)** — 기간 내 미소진분은 차기 회차로 이월. **2회차 분할 운영** (회차당 20장). 보상 구성 = 모바일 티켓 + 재화(CP·하트·셔플·폭탄·라인·시간+)만. 티켓 확률은 초안 2%(박스 1,000)가 너무 낮다는 판단으로 **티켓 20장을 유지한 채 박스를 570으로 축소해 3.5%로 상향** (총 지급량 계획 불변).

**회차당 표준 구성 (관리자 새 럭키드로우 기본 템플릿에 반영됨)**:

| 등급 | 보상 | 수량 | 확률 |
|---|---|---|---|
| S | 모바일 티켓 (1인 상한 2) | 20 | 3.5% |
| A | 500 CP | 25 | 4.4% |
| B | 폭탄 / 라인 / 시간+ | 35 / 35 / 40 | 6.1 / 6.1 / 7.0% |
| C | 300 CP / 셔플 | 52 / 70 | 9.1 / 12.3% |
| D | 150 CP / 하트 / 50 CP | 95 / 90 / 108 | 16.7 / 15.8 / 18.9% |
| | **합계 (박스 크기)** | **570** | 100% |

체감 지표: 평균 29회(≈14,500 CP)에 티켓 1장 기대, 20회 뽑으면 절반이 당첨, 10회 안 당첨 확률 30%.
...
3. **재화 기대값**: 유상 티켓 500 CP 대비 회수율 20%대 유지 — 티켓 기회가 메인 보상이므로 재화는 보조
4. 아이템(하트·부스터) 비중 ~45% — CP보다 게임 순환·리텐션에 유리
5. 1회차 소진 데이터(1인 평균 뽑기 횟수·소진 속도)로 2회차 박스 크기 보정 (박스가 작아진 만큼 소진·조기 종료가 빨라질 수 있음 — 소진 시 자동 종료는 정상 동작)
