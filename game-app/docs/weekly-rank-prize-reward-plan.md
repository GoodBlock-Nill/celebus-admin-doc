...
### 3-1-1. 운영 확률 설계 가이드 (2026-08-12 확정 — 콘서트 모바일 티켓 이벤트)

**확정 사항**: 모바일 티켓 총 40장 = **최대 지급량(예산 상한)** — 기간 내 미소진분은 차기 회차로 이월. **2회차 분할 운영** (회차당 20장). 보상 구성 = 모바일 티켓 + 재화(CP·하트·셔플·폭탄·라인·시간+)만.

**회차당 표준 구성 (관리자 새 럭키드로우 기본 템플릿에 반영됨)**:

| 등급 | 보상 | 수량 | 확률 |
|---|---|---|---|
| S | 모바일 티켓 (1인 상한 2) | 20 | 2.0% |
| A | 500 CP | 30 | 3.0% |
| B | 폭탄 / 라인 / 시간+ | 60 / 60 / 70 | 6 / 6 / 7% |
| C | 300 CP / 셔플 | 90 / 120 | 9 / 12% |
| D | 150 CP / 하트 / 50 CP | 180 / 160 / 210 | 18 / 16 / 21% |
| | **합계 (박스 크기)** | **1,000** | 100% |

**설계 원칙** (수량 조정 시에도 유지):
1. **위계**: 티켓은 전체에서 유일한 최저 확률 — "500 CP 수량 ≥ 티켓 수량 × 1.25" 규칙으로 보장
2. **박스 크기 = 예상 티켓 발행량 × 0.8** (기간 내 ~80% 소진 목표). 주간 발행량 ≈ [10 + 5×4 + 3×5 + (참여자−10)] × 2모드
3. **재화 기대값 ≈ 100~110 CP/뽑기** (유상 티켓 500 CP 대비 회수율 20%대) — 티켓 기회가 메인 보상이므로 재화는 보조
4. 아이템(하트·부스터) 비중 ~45% — CP보다 게임 순환·리텐션에 유리
5. 1회차 소진 데이터(1인 평균 뽑기 횟수·소진 속도)로 2회차 박스 크기 보정
