...
- **결정 이력**:
  - 2026-08-19 (v2.8): ⑳ **소진 속도 보정** — 2회차(티켓 5·박스 280)가 수일 내 95% 소진. 3회차부터 **티켓 5장 유지 + 재화만 증량**(1인 상한 1로 당첨자 5명 분산). 최초 ×2(박스 555) → **최소 1주 유지 목표로 ×2.5(박스 694, 티켓 0.72%) 확대**. "소진 속도는 재화 수량으로, 확률은 박스 크기로" + "게시 후 증량 불가 — 부족보다 잉여" 원칙 추가 (§3-1-1)
...
**운영 이력**: 1회차(티켓 20장·박스 570·3.5%) → 2회차는 운영자가 티켓 5장·박스 280으로 축소 운영 → **박스 280이 수일 내 95% 소진**(수요 대비 박스 과소). 3회차부터는 **티켓 5장 유지 + 재화 수량 증량**으로 확정 (2026-08-19 사용자 결정, 최초 ×2 → **최소 1주 유지 목표로 ×2.5 확대**): 2회차 재화 믹스를 그대로 2.5배 스케일해 경제 밸런스(뽑기당 기대값 ≈87 CP)는 유지하고 지속 기간을 연장. 일평균 ~100회 수요까지 7일 유지. **게시 후에는 재고 증량이 불가하므로 박스는 부족보다 잉여 쪽으로** (잉여는 잔여 공시·차기 이월로 무해, 조기 소진은 남은 기간 이벤트 공백).

**회차당 표준 구성 (관리자 새 럭키드로우 기본 템플릿에 반영됨)**:

| 등급 | 보상 | 수량 | 확률 |
|---|---|---|---|
| S | 모바일 티켓 (1인 상한 1) | 5 | 0.72% |
| A | 500 CP | 33 | 4.8% |
| B | 폭탄 / 라인 / 시간+ | 45 / 45 / 50 | 6.5 / 6.5 / 7.2% |
| C | 300 CP / 셔플 | 65 / 88 | 9.4 / 12.7% |
| D | 150 CP / 하트 / 50 CP | 120 / 113 / 130 | 17.3 / 16.3 / 18.7% |
| | **합계 (박스 크기)** | **694** | 100% |
