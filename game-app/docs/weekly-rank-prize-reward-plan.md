...
- **상태**: Phase 4 + 모바일 티켓(v2.2) 구현 완료 (2026-08-12) — 핵심 루프 완성. Phase 5 (운영 편의: 대시보드 배지·발행량 모니터링) 잔여
- **구현 이력**:
  ...
  - 2026-08-12 모바일 티켓(v2.2): 마이그레이션 040 — 풀 아이템 지급 방식(`fulfillment`: delivery/mobile_ticket) + 뽑기 RPC가 모바일 티켓 당첨을 즉시 지급 대기(`submitted`)로 생성하고 스냅샷에 CELEBUS 계정 식별자 보존. 유저: 입력 폼 없이 "일정 확정 후 CELEBUS 앱 지급" 안내, 기한 무효 미적용. 관리자: 풀 편집 지급 방식 선택(모바일 시 주소 옵션 숨김), 당첨자 테이블에 모바일 뱃지·CELEBUS 계정 표시·[지급 완료] 처리. dev 검증 + 브라우저 E2E 통과
...
- **결정 이력**:
  - 2026-08-19 (v2.8): ⑳ **소진 속도 보정** — 2회차(티켓 5·박스 280)가 수일 내 95% 소진. 3회차부터 **티켓 5장 유지 + 재화만 2배 증량**(박스 555, 티켓 0.90%, 1인 상한 1로 당첨자 5명 분산). "소진 속도는 재화 수량으로, 확률은 박스 크기로" 원칙 추가 (§3-1-1)
  - 2026-08-13 (v2.7): ⑲ **티켓 확률 2% → 3.5% 상향** — 평균 50회(≈25,000 CP)는 너무 무겁다는 판단. 티켓 20장(총 지급량)은 유지하고 **박스를 1,000 → 570으로 축소** (평균 29회·절반이 20회 안 당첨). 확률 조정은 티켓 수량이 아닌 박스 크기로 한다는 원칙 확립 (§3-1-1)
  ...
  - 2026-08-12 (v2.6): ⑱ **콘서트 티켓 이벤트 확률 확정** — 모바일 티켓 총 40장(최대 지급량, 미소진 이월)을 **2회차 분할**(회차당 20장·박스 1,000개·티켓 2%, 1인 상한 2). 티켓 = 전체 유일 최저 확률(위계), 재화 기대값 ≈ 105 CP(회수율 21%). §3-1-1 운영 확률 설계 가이드 + 관리자 기본 템플릿 반영
  - 2026-08-12 (v2.5): ⑰ **결과 카드 디자인 개편** — 유저 화면에 등급(S~D) 글자 비노출 (내부 분류·색 연출용으로만 유지). 카드 앞면은 **관리자가 풀 아이템별로 업로드한 카드 이미지**(세로형 5:7 권장, 3MB, 전용 버킷)로 전체를 채우고, 카드 안에는 텍스트를 두지 않으며 **보상명은 카드 하단(밖)에 표기**. 이미지 미등록 시 보상 아트(코인·아이템) 폴백
  - 2026-08-12 (v2.4): ⑯ **이벤트 생성 단일화** — 재화 뽑기/실물 박스를 나눠 생성하지 않고, 하나의 럭키드로우에 **실물(모바일 티켓·배송)과 재화(CP·아이템) 보상을 행 단위로 혼합 등록**. 모든 신규 이벤트는 수량 기반 단일 모델(잔여 수량 비례 균등 확률·소진 시 자동 종료·공시는 잔여/전체로 통일). 상시 재화 뽑기는 대수량 등록으로 운영. 기존 가중치 확률형(재화 확률형) 이벤트는 구형으로 계속 동작·편집 가능하나 신규 생성 불가
  - 2026-08-12 (v2.3): ⑭ **드로우 티켓 단일화** — 무상/유상 사용처 구분이 유저에게 과도하게 어렵다는 판단으로 폐지. 모든 뽑기(실물 박스 포함)에서 보유 티켓을 종류 구분 없이 사용 (내부 원장의 출처 추적은 유지). **⚠️ 조건: 기존 구분은 "CP 구매분 → 실물 경품" 사행성 차단 장치였음. 현재 CP는 무료 획득 재화라 수용하지만, CP 실결제(CELB/IAP) 도입 시 유료 재화로 실물을 뽑는 구조가 되므로 법무 재검토 필수** ⑮ **유저 노출 명칭 변경** — "가챠" → **럭키드로우 (Lucky Draw / ラッキードロー)**, "가챠 이용권" → **드로우 티켓 (Draw Ticket / ドローチケット)**. K-pop 팬덤(앨범·팬사인회 럭키드로우 이벤트)에 이미 친숙한 용어로 3개 언어권 자연스러움 + "가챠"의 사행성 뉘앙스 회피. 본 문서의 "가챠/이용권" 표기는 내부 설계 용어로 유지
  - 2026-08-12 (v2.2): ⑬ 실물 상품에 **지급 방식 구분** 추가 — 배송 / **모바일 티켓**. 콘서트 티켓은 배송이 아니라 **콘서트 일정 확정 후 CELEBUS 앱으로 입장용 모바일 티켓 지급**. 모바일 티켓 당첨은 수령 정보 입력이 불필요(CELEBUS 계정으로 지급)하므로 당첨 즉시 "지급 대기" 확정 — 기한 미입력 무효 개념 미적용. 당첨 스냅샷에 CELEBUS 계정 식별자를 보존해 운영자가 지급 대상을 확인
...
### 사행성 분리 원칙 → v2.3에서 티켓 단일화로 개정 (법률 유의)

당초 설계(v2)는 "대가를 지불하고, 우연에 의해, 재산상 이익(실물)을 얻는" 사행행위 구조를 피하기 위해 무상(랭킹 보상)/유상(CP 구매) 이용권을 분리하고 실물 뽑기를 무상 전용으로 강제했다. **v2.3에서 유저 이해도를 이유로 이 구분을 폐지하고 티켓을 단일 풀로 통합**했다 (사용자 결정 2026-08-12).

현재 수용 근거와 조건:
- 현재 CP는 게임 플레이·출석으로만 얻는 **무료 재화**이고, 콘서트 티켓도 **무료 공연 초대권**이라 "유료 대가 → 재산상 이익" 구조가 성립하지 않음
- **⚠️ CP 실결제(CELB/IAP)가 도입되는 시점에는 "현금 → CP → 티켓 → 실물 뽑기" 경로가 열리므로 법무 재검토 필수** — 재검토 전까지 실결제와 실물 뽑기를 동시에 활성화하지 말 것 (내부 원장은 무상/유상 출처를 계속 기록하므로 분리 복원 가능)
...
| 등급 | 아이템 | 수량 | 1인 상한 | 지급 방식 |
|---|---|---|---|---|
| S | V01D 콘서트 모바일 티켓 (무료 공연) | 10 | **2매** | **모바일 티켓** — 일정 확정 후 CELEBUS 앱 지급 (수령 정보 불요) |
| A | 멤버 사인 굿즈 | 5 | - | 배송 (주소 필요) |
| B | 한정 포토카드 세트 | 50 | - | 배송 |
| C | 100 CP | 150 | - | 재화 (즉시 지급) |
| D | 하트 1개 | 285 | - | 재화 (즉시 지급) |

- **1인 당첨 상한 (per-user cap)**: 아이템별로 설정 가능. 콘서트 티켓은 **1인 최대 2매** — 상한에 도달한 유저의 뽑기에서는 해당 아이템이 추첨 대상에서 제외되고 잔여 풀에서 추첨된다 (티켓이 특정인에게 쏠리는 것 방지, 2매 = 동반 1인 관람 배려)
- **지급 방식 (v2.2)**: 실물 상품은 **배송**(수령 정보 입력 필요) / **모바일 티켓**(콘서트 일정 확정 후 CELEBUS 앱으로 입장 티켓 지급 — 당첨자의 CELEBUS 계정으로 지급하므로 수령 정보 입력·기한 무효 개념 없음, 당첨 즉시 "지급 대기" 확정) 중 선택
...
### 3-1-1. 운영 확률 설계 가이드 (2026-08-12 확정 · 08-13 확률 상향 · 08-19 소진 속도 보정 — 콘서트 모바일 티켓 이벤트)

**운영 이력**: 1회차(티켓 20장·박스 570·3.5%) → 2회차는 운영자가 티켓 5장·박스 280으로 축소 운영 → **박스 280이 수일 내 95% 소진**(수요 대비 박스 과소). 3회차부터는 **티켓 5장 유지 + 재화 수량 증량**으로 확정 (2026-08-19 사용자 결정): 2회차 재화 믹스를 그대로 2배 스케일해 경제 밸런스(뽑기당 기대값 ≈86 CP)는 유지하고 지속 기간만 2배로 연장.

**회차당 표준 구성 (관리자 새 럭키드로우 기본 템플릿에 반영됨)**:

| 등급 | 보상 | 수량 | 확률 |
|---|---|---|---|
| S | 모바일 티켓 (1인 상한 1) | 5 | 0.90% |
| A | 500 CP | 26 | 4.7% |
| B | 폭탄 / 라인 / 시간+ | 36 / 36 / 40 | 6.5 / 6.5 / 7.2% |
| C | 300 CP / 셔플 | 52 / 70 | 9.4 / 12.6% |
| D | 150 CP / 하트 / 50 CP | 96 / 90 / 104 | 17.3 / 16.2 / 18.7% |
| | **합계 (박스 크기)** | **555** | 100% |

1인 상한을 2 → 1로 내려 티켓 5장이 서로 다른 5명에게 돌아가게 한다 (희소 회차의 당첨자 분산).

**설계 원칙** (수량 조정 시에도 유지):
1. **위계**: 티켓은 전체에서 유일한 최저 확률 — "500 CP 수량 ≥ 티켓 수량 × 1.25" 규칙으로 보장
2. **확률 조정은 박스 크기로**: 티켓 확률을 올리고/내리고 싶으면 티켓 수량(=총 지급량)이 아니라 박스 크기를 줄이고/늘린다 — 예산 상한이 흔들리지 않음
3. **소진 속도 조정은 재화 수량으로**: 회차가 너무 빨리 끝나면 티켓이 아닌 재화 행을 배수 스케일한다 — 믹스 비율을 유지하면 경제 밸런스(기대값)가 그대로 보존됨
4. **재화 기대값**: 유상 티켓 500 CP 대비 회수율 20%대 유지 — 티켓 기회가 메인 보상이므로 재화는 보조
5. 아이템(하트·부스터) 비중 ~45% — CP보다 게임 순환·리텐션에 유리
6. 직전 회차 소진 데이터(소진 일수·1인 평균 뽑기 횟수)로 다음 회차 박스 크기 보정 (소진 시 자동 종료는 정상 동작)
