-- ─────────────────────────────────────────────────────────────
-- 015: V01D 멤버 랭킹 — 멤버 지정(관리자) + 리더보드 멤버 표시 + 멤버 보드(주간·월간)
-- ─────────────────────────────────────────────────────────────

alter table game_profiles add column if not exists is_member boolean not null default false;

-- 기간 리더보드 재작성 — 멤버 플래그 포함 (011 대체, 시그니처 불변)
...
language sql stable security definer set search_path = public, extensions as $$
  ...
  select coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb) from (
    select row_number() over (order by best.level desc, best.score desc, best.created_at asc) as rank,
           best.nickname, best.avatar, best.level, best.score,
...
-- V01D 멤버 보드 — 멤버별 주간·월간 최종 성적 + 전체 보드 내 순위 (anon 공개)
...
begin
  ...
  for m in select player_hash, nickname, avatar from game_profiles where is_member order by nickname loop
    -- 주간
    select jsonb_build_object('rank', r.rank, 'level', r.level, 'score', r.score) into v_week
    from (
      with best as (
        select distinct on (player_hash) player_hash, level, score, created_at
      ...
      select me.level, me.score,
             (select count(*) + 1 from best b
               where (b.level, b.score, -extract(epoch from b.created_at)) > (me.level, me.score, -extract(epoch from me.created_at)))::int as rank
      from best me where me.player_hash = m.player_hash
    ) r;
    -- 월간
    select jsonb_build_object('rank', r.rank, 'level', r.level, 'score', r.score) into v_month
    from (
      with best as (
        select distinct on (player_hash) player_hash, level, score, created_at
    ...
    v_out := v_out || jsonb_build_array(jsonb_build_object(
      'nickname', m.nickname, 'avatar', m.avatar,
      'week', v_week, 'month', v_month));
...
-- 관리자: 멤버 지정/해제
create or replace function admin_set_member(p_h text, p_on boolean)
returns jsonb language plpgsql security definer set search_path = public, extensions as $$
begin
  update game_profiles set is_member = p_on where player_hash = p_h;
  if not found then return jsonb_build_object('error', 'not_found'); end if;
  return jsonb_build_object('ok', true, 'is_member', p_on);
end $$;
revoke execute on function admin_set_member(text, boolean) from public, anon, authenticated;
grant  execute on function admin_set_member(text, boolean) to service_role;

-- 관리자 회원 검색/상세 재작성 — is_member 포함
create or replace function admin_search_profiles(p_q text, p_limit int default 30)
returns jsonb language sql security definer set search_path = public, extensions as $$
  select coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb) from (
    select player_hash, nickname, phone_cc, phone, avatar, created_at, last_login_at, is_member
    from game_profiles
