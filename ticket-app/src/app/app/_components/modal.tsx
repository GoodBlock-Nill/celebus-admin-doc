  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="relative w-full max-w-[420px] rounded-t-3xl border-t border-[#2A2C34] bg-[#191A20] px-5 pb-7 pt-6"
      >
        <h2 className="text-[16px] font-bold">{title}</h2>
        {description ? (
          <div className={`mt-2 text-[13px] leading-relaxed ${MUTED}`}>{description}</div>
        ) : null}
