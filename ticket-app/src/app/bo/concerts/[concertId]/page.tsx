...
import { Card, Collapsible, DefinitionRow, EmptyState, PageHeader, StatusBadge } from '../../_components/ui';
...
export default function ConcertDetailPage() {
  ...
  const logs = useTicketStore((state) => state.logs);

  const concert = concerts.find((item) => item.id === concertId);
  const ownSessions = useMemo(
    () => sessions.filter((session) => session.concertId === concertId),
    [sessions, concertId],
  );
  ...
  const relatedLogs = useMemo(() => {
    ...
    const keywords = [
      concert.title,
      ...ownSessions.map((session) => session.name),
      ...orders.filter((order) => order.concertId === concertId).map((order) => order.orderNo),
      ...tickets.filter((ticket) => ticket.concertId === concertId).map((ticket) => ticket.code),
    ];
    return logs
      .filter((log) => keywords.some((keyword) => keyword && log.detail.includes(keyword)))
      .slice(0, RECENT_LOG_LIMIT);
  }, [concert, ownSessions, orders, tickets, logs, concertId]);

  const logColumns: Array<Column<ActivityLog>> = [
    {
      key: 'at',
      header: '시각',
      width: '150px',
    ...
    { key: 'action', header: '액션', width: '140px', render: (log) => <span className="font-semibold">{log.action}</span> },
  ...
  if (!concert) {
    return (
      <>
        <PageHeader title="공연 상세" />
        <Card>
          <EmptyState text="공연 정보를 찾을 수 없습니다." />
          <div className="mt-3">
            <Link href="/bo/concerts" className="text-[13px] font-semibold text-[#3056D3] hover:underline">
              공연 목록으로 돌아가기
            </Link>
          </div>
        </Card>
    ...
    <>
      ...
      <Card title="공연 정보">
        ...
        <div className="mt-4 flex flex-col gap-2">
          ...
          <Collapsible summary="취소·환불 규정 원문">
            <p className="whitespace-pre-line text-[13px] leading-relaxed text-[#4A4E5A]">
      ...
      <Card title="배정 수량 재조정" description="분류 사이에서 배정 수량을 옮깁니다.">
        {ownSessions.length > 0 ? (
          <ReallocateForm sessions={ownSessions} />
        ) : (
          <EmptyState text="등록된 회차가 없습니다." />
        )}
      </Card>

      <Card title="무상 티켓 발급" description="래플 당첨자 지급·IX 초대 명단 지급 모의">
        {ownSessions.length > 0 ? (
          <CompIssueForm sessions={ownSessions} />
        ) : (
          <EmptyState text="등록된 회차가 없습니다." />
        )}
      </Card>
