...
const ACTIONS: ReportActionType[] = [
  '노출 차단',
  '수사기관 제출',
  '계정 제재',
  '티켓 무효화',
    ...
    <article className="rounded-xl border border-[#E3E5EA] bg-white p-5 shadow-[0_1px_2px_rgba(27,29,34,0.04)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1.5 whitespace-pre-line text-[13px] leading-relaxed text-[#4A4E5A]">
            {report.detail}
          </p>
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[12px] text-[#6B7080]">
            ...
            {report.evidenceUrl ? (
              <span className="break-all">증빙 {report.evidenceUrl}</span>
            ) : null}
          </div>
        </div>
      ...
      {report.actions.length > 0 ? (
        <ol className="mt-3 flex flex-col gap-1 border-t border-[#F0F1F4] pt-3">
          {report.actions.map((action, index) => (
            <li key={`${action.type}-${action.at}-${index}`} className="flex items-center gap-2 text-[12px]">
              <span className="text-[#3056D3]">●</span>
              <span className="font-semibold text-[#1B1D22]">{action.type}</span>
              <span className="tabular-nums text-[#6B7080]">{formatDateTime(action.at)}</span>
            </li>
          ))}
        </ol>
      ) : null}
