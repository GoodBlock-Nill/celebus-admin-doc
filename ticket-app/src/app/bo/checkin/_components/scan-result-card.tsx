...
import type { CheckInResult, ConcertSession, Concert } from '@/lib/types';
...
export const SCAN_VIEW: Record<CheckInResult['kind'], ScanView> = {
  ...
  if (!result) {
    return (
      <div className="rounded-xl border border-dashed border-[#E3E5EA] bg-[#FAFBFC] px-4 py-10 text-center text-[13px] text-[#6B7080]">
        티켓 코드를 입력하면 결과가 여기에 표시됩니다.
      </div>
    );
  }

  const view = SCAN_VIEW[result.kind];
  const ticket = result.kind === 'INVALID' ? undefined : result.ticket;
  const concert = ticket ? concerts.find((item) => item.id === ticket.concertId) : undefined;
  const session = ticket ? sessions.find((item) => item.id === ticket.sessionId) : undefined;
    ...
    <div className={`rounded-xl border-2 px-5 py-5 ${CARD_STYLE[view.tone]}`}>
      ...
      {ticket ? (
        <div className="mt-4 flex flex-col gap-1.5 border-t border-black/10 pt-3">
          ...
          <Row label="공연" value={concert?.title ?? '-'} />
          <Row label="회차" value={session?.name ?? '-'} />
