  ...
  return (
    <aside className="sticky top-0 flex h-dvh w-[220px] shrink-0 flex-col border-r border-[#E3E5EA] bg-white">
      ...
      <div className="border-t border-[#E3E5EA] px-4 py-4">
        <p className="text-[11px] font-bold tracking-[0.08em] text-[#6B7080]">처리자</p>
        <p className="mt-1 text-[14px] font-bold text-[#1B1D22]">{adminName}</p>
        <p className="mt-1 text-[11px] leading-relaxed text-[#6B7080]">
          모든 처리는 이 이름으로 활동 로그에 기록됩니다.
        </p>
        <button
          type="button"
          onClick={() => void handleLogout()}
          className="mt-3 w-full rounded-lg border border-[#C9CDD6] bg-white px-3 py-2 text-[12px] font-semibold text-[#1B1D22] transition-colors hover:bg-[#F2F3F6]"
        >
          로그아웃
        </button>
      </div>
