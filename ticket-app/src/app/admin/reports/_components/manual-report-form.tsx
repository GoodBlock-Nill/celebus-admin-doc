...
export function ManualReportForm({ onDone }: { onDone: () => void }) {
  const toast = useToast();

  const [targetType, setTargetType] = useState<ReportTargetType>('외부 링크');
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true);
    const result = await adminApi.submitReport({
      targetType,
      reason: reason.trim(),
      detail: detail.trim(),
    ...
    toast.fromResult(result, '외부 통보 건을 접수했습니다. 처리 기한 10시간이 시작됩니다.');
    if (result.ok) {
      setReason('');
      setDetail('');
      setEvidenceUrl('');
