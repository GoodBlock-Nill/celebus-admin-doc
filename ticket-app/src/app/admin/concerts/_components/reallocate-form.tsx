...
import { POOL_TYPES, isPoolType } from '../../_components/pools';
import { useToast } from '../../_components/toast';
import { InfoNote } from '../../_components/ui';
import { adminApi } from '@/lib/admin-client';
import type { AdminSessionView } from '@/lib/admin-types';
import { poolLabel, type PoolType } from '@/lib/api-types';
...
export function ReallocateForm({
  ...
  const fromStock = session?.pools.find((pool) => pool.poolType === from);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const result = await adminApi.reallocate({
      concertId,
