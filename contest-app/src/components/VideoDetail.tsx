...
export default function VideoDetail({ postId }: { postId: string }) {
  ...
  const focus = useSearchParams().get("focus"); // 알림 딥링크: comment
  ...
  const [memberHearted, setMemberHearted] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/stage/mine?liked_for=${postId}&member_heart_for=${postId}`);
      const j = await res.json();
      setLiked((j.liked ?? []).includes(postId));
      setMemberHearted((j.memberHearted ?? []).includes(postId));
    } catch {
      /* 좋아요·하트 상태는 보조 정보 */
    }
    setLoading(false);
  ...
  // 알림 딥링크 — 로드 후 댓글 위치로 스크롤 + 잠깐 강조
  useEffect(() => {
    if (loading || !post || focus !== "comment") return;
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        {isMemberMe && (
          <button onClick={toggleMemberHeart} aria-pressed={memberHearted} className="brand-gradient flex min-h-11 items-center gap-1.5 rounded-full px-4 py-2.5 text-[13px] font-bold text-white shadow-sm">
            <Heart className={`h-4 w-4 ${memberHearted ? "fill-current" : ""}`} /> {t("mh_button")}
          </button>
        )}
