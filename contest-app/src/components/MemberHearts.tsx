...
function MemberAvatar({ heart, size }: { heart: MemberHeartPublic; size: number }) {
  return heart.avatar_url ? (
    ...
    <img
      ...
      className="rounded-full object-cover ring-2 ring-[#141217]"
    />
  ) : (
    <span
      style={{ width: size, height: size, fontSize: size * 0.42 }}
      className="flex items-center justify-center rounded-full bg-primary/30 font-bold text-primary-400 ring-2 ring-[#141217]"
    >
...
// 카드용 컴팩트 스택 (썸네일 위 오버레이)
export function MemberHeartStack({ hearts }: { hearts: MemberHeartPublic[] }) {
  if (hearts.length === 0) return null;
  const shown = hearts.slice(0, 3);
  return (
    <span className="flex items-center gap-1 rounded-full bg-black/65 py-0.5 pl-1 pr-2">
...
export function MemberHeartsLine({ hearts }: { hearts: MemberHeartPublic[] }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-xl bg-primary/10 px-3 py-2.5 ring-1 ring-primary/25">
      <span className="flex -space-x-2">
        {sorted.slice(0, 5).map((h) => (
          <MemberAvatar key={h.member_id} heart={h} size={26} />
        ))}
      </span>
      <span className="min-w-0 flex-1 text-[12.5px] font-bold leading-snug text-primary-400">{text}</span>
    </div>
  );
}
