  ...
  return (
    <div>
      <div className="mb-4">
        ...
        <p className="mt-0.5 text-[12.5px] text-muted">{t("ev_list_sub")}</p>
      </div>
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 2 }).map((_, i) => (
            <div key={i} className="h-28 animate-pulse rounded-2xl border border-border bg-card-2" />
          ))}
        </div>
      ) : events.length === 0 ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`shrink-0 rounded-full px-2 py-0.5 text-[10.5px] font-bold ${
                    e.status === "open" ? "bg-primary-soft text-primary-strong" : "bg-amber-50 text-amber-700"
                  }`}
                >
