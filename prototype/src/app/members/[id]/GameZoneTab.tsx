...
import { formatDateTime, formatDate, formatGP, formatNumber } from '@/lib/utils';
...
export default function GameZoneTab({ member }: GameZoneTabProps) {
  ...
  const winRate = completedGames.length > 0
    ...
    : null;
  const latestParticipation = participants.length > 0
    ? participants.sort((a, b) => new Date(b.participatedAt).getTime() - new Date(a.participatedAt).getTime())[0].participatedAt
    : null;

  // Filter game participants
  ...
  // Game participants table columns
  const gameColumns: Column<EnrichedParticipant>[] = [
    {
      key: 'gameTitle',
      label: '게임 타이틀',
      width: '200px',
    ...
    },
    {
      ...
      render: (p: EnrichedParticipant) => {
        ...
        if (p.choice === p.game.result) {
          return <span className="text-green-600">정답({p.game.result})</span>;
        }
    ...
    { key: 'refundGP', label: '환급 GP', width: '90px', align: 'right', sortable: true, render: (p: EnrichedParticipant) => formatGP(p.refundGP) },
    {
      ...
      width: '90px',
      align: 'center',
      render: (p: EnrichedParticipant) => <Badge variant="gameStatus" value={p.game?.status || 'Active'} />
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <StatsCard label="참여 게임 수" value={`${enrichedParticipants.length}회`} />
          <StatsCard label="승률" value={winRate !== null ? `${winRate}%` : '-'} />
          <StatsCard label="최근 참여일" value={latestParticipation ? formatDate(latestParticipation) : '-'} />
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="mb-4">
          <FilterBar
            filters={[
              { key: 'gameStatus', label: '게임 상태', type: 'select', value: gameStatusFilter, options: [
