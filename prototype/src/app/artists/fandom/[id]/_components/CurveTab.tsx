...
export default function CurveTab({ fandom, onEditingChange }: { fandom: FandomLevel; onEditingChange?: (v: boolean) => void }) {
  ...
  const removeLevel = (i: number, level: number) => {
    // 현재 레벨 이하(진입한 레벨)는 삭제 불가 — [CEB-BO-EVT-201-EDIT] §4.3
    if (isAchieved(level)) { toast.error('현재 레벨 이하는 수정·삭제할 수 없습니다.'); return; }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-5">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5">
        ...
        <table className="w-full text-sm">
          <thead className="bg-indigo-50/60 border-y border-indigo-100">
            <tr>
              ...
              <th className="px-4 py-2.5 text-center text-xs font-semibold text-gray-700 w-[90px]">레벨업 여부</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-700 w-[150px]">레벨업 일시</th>
              ...
              return (
                <tr key={i} className={achieved ? 'bg-gray-50/60' : ''}>
                  ...
                  <td className="px-4 py-2.5 text-center">
                    {achieved ? (
                      <span className="inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium bg-emerald-100 text-emerald-700">레벨업 완료</span>
                    ) : (
                      <span className="inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium bg-gray-100 text-gray-500">미완료</span>
                    )}
                  </td>
        ...
        <p className="mt-3 text-xs text-gray-400">진행중 시즌에도 현재 레벨을 초과한(미진입) 레벨을 추가할 수 있습니다. 신규 레벨 보상은 추가 이후 그 레벨에 진입하는 시점부터 적용되며 기존 지급분은 변경되지 않습니다.</p>
      </div>
