...
export default function CurveTab({ fandom }: { fandom: FandomLevel }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-5">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setSaveOpen(true)} className="h-9 px-3 text-xs font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">저장</button>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-indigo-50/60 border-y border-indigo-100">
            <tr>
              ...
              <th className="px-4 py-2.5 w-[48px]" />
            </tr>
          </thead>
      ...
      <ConfirmModal isOpen={saveOpen} onClose={() => setSaveOpen(false)} onConfirm={() => { setSaveOpen(false); toast.success(`'${fandom.groupName}'의 팬덤 레벨 곡선을 저장했습니다.`); }} title="곡선을 저장할까요?" lines={['변경된 레벨 곡선이 앱에 반영됩니다.']} confirmLabel="저장" />
    </div>
  );
}
