...
import type { RewardType, RelatedLink } from '@/lib/fq-types';
...
function getTomorrowMin(): string {
  const d = new Date();
...
export default function QuestCreatePage() {
  ...
  const createQuest = useFQStore((s) => s.createQuest);
  ...
  }, [title, description, userGuide, thumbnailImage, artist, questType, deadlineValid, linksValid, nftValid]);
  ...
  const handleCreate = () => {
    ...
    router.push(`/fan-quest/quests/${newQuest.id}`);
  };
  ...
  return (
    <div>
          ...
          <div className="space-y-5">
            ...
            <div>
              <DateTimePicker
                ...
                required
                min={`${getTomorrowMin()}T00:00:00.000Z`}
              />
      ...
      <ConfirmModal
        ...
        title="Quest 생성"
        message="확인 버튼을 누르면 Quest가 생성됩니다. 계속 진행하시겠습니까?"
        confirmText="확인"
        cancelText="취소"
      />
    </div>
...
function Dropdown({ label, value, onChange, options, optionLabels, placeholder, required }: DropdownProps) {
  return (
    <div>
...
function MultiLangInputWithStatus({ label, values, onChange, maxLength, required }: MultiLangInputWithStatusProps) {
...
function MultiLangTextareaWithStatus({ label, values, onChange, maxLength, rows = 4, required }: MultiLangTextareaWithStatusProps) {
  const [activeLang, setActiveLang] = useState<'ko' | 'en' | 'jp'>('ko');
