...
export default function HomePage() {
  ...
  const handleCheckIn = useCallback(() => {
    ...
    setCheckedIn(true);
    setStreak((s) => s + 1);
    addToast('success', '출석 완료! 덕력 5DUK 획득');
          ...
          <div className="rounded-2xl overflow-hidden">
            ...
            <button onClick={handleCheckIn} disabled={checkedIn} className={cn(
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-xs text-white/70 mt-0.5">{checkedIn ? `🔥 ${streak}일 연속 · +5DUK 획득` : `탭하여 출석 · 덕력 +5DUK`}</p>
              </div>
              {!checkedIn && (
            ...
            <button onClick={() => router.push('/daily-mission')} className="w-full bg-gray-50 px-5 py-3.5 flex items-center gap-3 active:bg-gray-100 transition-colors">
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-xs text-gray-400 mt-0.5">{missionDone ? '+25DUK 획득' : '오늘의 모든 미션을 완료하면 덕력 25DUK 일괄 지급'}</p>
              </div>
